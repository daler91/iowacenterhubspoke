...
async def ensure_indexes(db, logger):
    ...
    try:
        ...
        await _ensure(
            db.activity_logs,
            [[("timestamp", -1)], [("entity_type", 1), ("entity_id", 1)], "expires_at", [("user_id", 1)]],
        )
        ...
        await _ensure(db.projects, [[("partner_org_id", 1)], [("phase", 1)], [("community", 1)], [("deleted_at", 1)], [("schedule_id", 1), ("deleted_at", 1)]])
        await _ensure(db.tasks, [[("project_id", 1)], [("project_id", 1), ("phase", 1)], [("project_id", 1), ("completed", 1)]])
        await _ensure(db.partner_orgs, [[("community", 1)], [("status", 1)], [("deleted_at", 1)], [("location_id", 1), ("status", 1), ("deleted_at", 1)]])
        await _ensure(db.partner_contacts, [[("partner_org_id", 1)]])
        await _ensure(db.task_attachments, [[("task_id", 1)]])
        await _ensure(db.task_comments, [[("task_id", 1), ("created_at", 1)], [("task_id", 1), ("parent_comment_id", 1)]])
        ...
        await _ensure(db.webhook_subscriptions, [[("active", 1), ("events", 1)], [("deleted_at", 1)]])
        await _ensure(db.schedules, [[("series_id", 1), ("date", 1)]])
        await _ensure(db.messages, [[("project_id", 1), ("visibility", 1)], [("project_id", 1), ("created_at", -1)]])
        ...
        await _ensure(db.notifications, [[("principal_kind", 1), ("principal_id", 1), ("created_at", -1)], [("principal_kind", 1), ("principal_id", 1), ("read_at", 1), ("dismissed_at", 1)]])
        await _ensure(db.notification_queue, [[("principal_kind", 1), ("principal_id", 1), ("frequency", 1), ("sent_at", 1)]])
        ...
        await db.notifications.create_index("created_at_date", expireAfterSeconds=60 * 60 * 24 * 365, partialFilterExpression=ttl_partial_filter)
        await db.notifications_sent.create_index("created_at_date", expireAfterSeconds=60 * 60 * 24 * 90, partialFilterExpression=ttl_partial_filter)
        await db.notification_queue.create_index("created_at_date", expireAfterSeconds=60 * 60 * 24 * 30, partialFilterExpression=ttl_partial_filter)
