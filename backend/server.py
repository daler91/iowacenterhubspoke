...
async def update_location(
    location_id: str, data: LocationUpdate, user=Depends(get_current_user)
):
...
async def create_employee(data: EmployeeCreate, user=Depends(get_current_user)):
    ...
    await log_activity(
        "employee_created",
        f"Employee '{data.name}' added to team",
        "employee",
        emp_id,
        user.get("name", "System"),
    )
...
async def update_employee(
    employee_id: str, data: EmployeeUpdate, user=Depends(get_current_user)
):
    update_data = {k: v for k, v in data.model_dump().items() if v is not None}
    if not update_data:
        raise HTTPException(status_code=400, detail="No fields to update")
...
async def create_schedule(data: ScheduleCreate, user=Depends(get_current_user)):
    ...
    doc = {
        ...
        "drive_time_minutes": (
            data.travel_override_minutes if data.travel_override_minutes else drive_time
        ),
...
async def update_schedule(
    schedule_id: str, data: ScheduleUpdate, user=Depends(get_current_user)
):
    update_data = {k: v for k, v in data.model_dump().items() if v is not None}
...
async def update_schedule_status(
    schedule_id: str, data: StatusUpdate, user=Depends(get_current_user)
):
    if data.status not in ["upcoming", "in_progress", "completed"]:
        raise HTTPException(status_code=400, detail="Invalid status")
    result = await db.schedules.update_one(
        {"id": schedule_id}, {"$set": {"status": data.status}}
    )
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Schedule not found")
    updated = await db.schedules.find_one({"id": schedule_id}, {"_id": 0})
...
async def get_activity_logs(limit: int = 30, user=Depends(get_current_user)):
    logs = (
        await db.activity_logs.find({}, {"_id": 0}).sort("timestamp", -1).to_list(limit)
    )
    return logs
...
async def get_employee_stats(employee_id: str, user=Depends(get_current_user)):
    ...
    all_schedules = await db.schedules.find(
    ...
    upcoming = sum(
        1 for s in all_schedules if s.get("status", "upcoming") == "upcoming"
    )
    ...
    return {
        ...
        "monthly_breakdown": [
            {"month": k, "count": v} for k, v in sorted(weekly.items())
        ],
        "recent_schedules": sorted(
            all_schedules, key=lambda x: x.get("date", ""), reverse=True
        )[:10],
    }
...
async def get_notifications(user=Depends(get_current_user)):
    ...
    t2t_schedules = await db.schedules.find({"town_to_town": True}, {"_id": 0}).to_list(
        100
    )
        ...
        notifications.append(
            {
                ...
                "description": s.get(
                    "town_to_town_warning", "Verify drive time manually"
                ),
                "severity": "warning",
                "timestamp": s.get("created_at", today),
                "entity_id": s["id"],
    ...
    return sorted(
        notifications, key=lambda x: x.get("severity") == "warning", reverse=True
    )
