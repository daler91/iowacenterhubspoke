...
async def run(db) -> int:
    ...
    await _ensure(
        db.activity_logs,
        [[("timestamp", -1)], [("entity_type", 1), ("entity_id", 1)], [("user_id", 1)]],
    )
    ...
    await _ensure(
        db.projects,
        [[("partner_org_id", 1)], [("phase", 1)], [("community", 1)], [("deleted_at", 1)], [("schedule_id", 1), ("deleted_at", 1)]],
    )
    await _ensure(db.tasks, [[("project_id", 1)], [("project_id", 1), ("phase", 1)], [("project_id", 1), ("completed", 1)]])
    await _ensure(
        db.partner_orgs,
        [[("community", 1)], [("status", 1)], [("deleted_at", 1)], [("location_id", 1), ("status", 1), ("deleted_at", 1)]],
    )
    ...
    await _ensure(
        db.notifications,
        [[("principal_kind", 1), ("principal_id", 1), ("created_at", -1)], [("principal_kind", 1), ("principal_id", 1), ("read_at", 1), ("dismissed_at", 1)]],
    )
    ...
    await db.notifications_sent.create_index(
        [("principal_kind", 1), ("principal_id", 1), ("type_key", 1), ("channel", 1), ("dedup_key", 1)],
        unique=True,
    )
