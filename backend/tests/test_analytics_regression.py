...
from core.math_utils import linear_regression  # noqa: E402


def test_perfect_positive_slope():
    # y = 2x + 1 over x = 0..4 → slope 2, intercept 1
    slope, intercept = linear_regression([1.0, 3.0, 5.0, 7.0, 9.0])
...
def test_perfect_negative_slope():
    ...
    slope, intercept = linear_regression([10.0, 7.0, 4.0, 1.0, -2.0])
    assert abs(slope - (-3.0)) < 1e-9
    assert abs(intercept - 10.0) < 1e-9


def test_flat_series_returns_zero_slope():
    slope, intercept = linear_regression([5.0, 5.0, 5.0, 5.0])
...
def test_insufficient_data():
    ...
    assert linear_regression([7.5]) == (0.0, 7.5)
