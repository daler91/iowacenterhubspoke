...
class HubSpokeAPITester:
            ...
            if success:
                ...
                try:
                    ...
                    if isinstance(response_data, dict) and len(str(response_data)) < 500:
                        ...
                        print("   Response: List with {len(response_data)} items")
    ...
    def test_auth_register(self):
        ...
        if success and 'token' in response:
            ...
            print("   Registered user: {test_email}")
            return True
        return False
    ...
    def test_auth_login(self):
        ...
        if success and 'token' in response:
            self.token = response['token']
            self.user_id = response.get('user', {}).get('id')
            print("   Logged in as: {response.get('user', {}).get('email')}")
            return True
        return False
    ...
    def test_locations_get(self):
        ...
        if success:
            if len(response) >= 5:
                print("   Found {len(response)} locations (expected 5+ seeded)")
                ...
                print("   Seeded cities found: {found_cities}")
                return len(found_cities) >= 4  # Allow for some flexibility
            else:
                print("   Only found {len(response)} locations, expected 5 seeded")
                return False
        return success
    ...
    def test_locations_create(self):
        ...
        if success and 'id' in response:
            ...
            print("   Created location ID: {self.created_location_id}")
        return success
    ...
    def test_employees_create(self):
        ...
        if success and 'id' in response:
            ...
            print("   Created employee ID: {self.created_employee_id}")
        return success
    ...
    def test_schedules_create(self):
        ...
        if success and 'id' in response:
            ...
            print("   Created schedule ID: {self.created_schedule_id}")
            # Check for drive time calculation
            if 'drive_time_minutes' in response:
                print("   Drive time calculated: {response['drive_time_minutes']} minutes")
        return success
    ...
    def test_schedules_town_to_town(self):
        ...
        if success:
            if response.get('town_to_town'):
                print("   ✅ Town-to-town detected: {response.get('town_to_town_warning')}")
            else:
                print("   ⚠️  Town-to-town not detected (might be expected)")
        return success
    ...
    def test_activity_logs(self):
        ...
        if success:
            print("   Found {len(response)} activity log entries")
            if len(response) > 0:
                ...
                if missing_keys:
                    print("   ⚠️  Missing keys in activity log: {missing_keys}")
                else:
                    print("   ✅ Activity log structure correct")
        return success
    ...
    def test_employee_stats(self):
        ...
        if success:
            ...
            if missing_keys:
                print("   ⚠️  Missing keys in employee stats: {missing_keys}")
                return False
            else:
                print("   ✅ Employee stats structure correct")
                print("   Stats: {response['total_classes']} classes, {response['completed']} completed")
        return success
    ...
    def test_notifications(self):
        ...
        if success:
            print("   Found {len(response)} notifications")
            if len(response) > 0:
                ...
                if missing_keys:
                    print("   ⚠️  Missing keys in notification: {missing_keys}")
                else:
                    print("   ✅ Notification structure correct")
                    print("   Types found: {list(set(n.get('type', 'unknown') for n in response))}")
        return success
    ...
    def test_workload_stats(self):
        ...
        if success:
            print("   Found workload data for {len(response)} employees")
            if len(response) > 0:
                ...
                if missing_keys:
                    print("   ⚠️  Missing keys in workload data: {missing_keys}")
                    return False
                else:
                    print("   ✅ Workload data structure correct")
        return success
    ...
    def test_schedule_check_conflicts(self):
        ...
        if success:
            ...
            missing_keys = [key for key in expected_keys if key not in response]
            if missing_keys:
                print("   ⚠️  Missing keys in conflict check: {missing_keys}")
                return False
            else:
                print("   ✅ Conflict check structure correct")
                print("   Has conflicts: {response.get('has_conflicts', False)}")
        return success
    ...
    def test_schedule_recurring(self):
        ...
        if success:
            if 'total_created' in response:
                print("   ✅ Created {response['total_created']} recurring schedules")
                if response.get('conflicts_skipped'):
                    print("   ⚠️  {len(response['conflicts_skipped'])} schedules skipped due to conflicts")
            else:
                print("   ✅ Single schedule created (no recurrence data)")
        return success
    ...
    def test_schedule_conflict_409(self):
        ...
        if success:
            print("   ✅ Conflict properly detected (409 returned)")
        return success
    ...
    def test_schedule_relocate(self):
        ...
        if success:
            if response.get('date') == new_date and response.get('start_time') == '14:00':
                print("   ✅ Schedule successfully relocated to {new_date} 14:00-17:00")
            else:
                print("   ⚠️  Schedule relocation may not have worked properly")
        return success
    ...
    def test_weekly_summary_report(self):
        ...
        if success:
            ...
            missing_keys = [key for key in expected_keys if key not in response]
            if missing_keys:
                print("   ⚠️  Missing keys in weekly report: {missing_keys}")
                return False
            else:
                print("   ✅ Weekly report structure correct")
                print("   Period: {response['period']['from']} to {response['period']['to']}")
                print("   Totals: {response['totals']['classes']} classes, {response['totals']['employees_active']} active employees")
                print("   Employee details: {len(response['employees'])} employees")
        return success
    ...
    def test_weekly_summary_custom_dates(self):
        ...
        if success:
            if response.get('period', {}).get('from') == start_date:
                print("   ✅ Custom date range working: {start_date} to {end_date}")
            else:
                print("   ⚠️  Custom date range may not be working properly")
        return success
