...
export default function CustomRecurrenceDialog({ open, onOpenChange, startDate, value, onSave }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] bg-white dark:bg-card" data-testid="custom-recurrence-dialog">
        <DialogHeader>
          ...
          <div className="grid grid-cols-[120px_120px_1fr] gap-3 items-end">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="bg-muted/50 dark:bg-muted"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={draft.frequency} onValueChange={(value) => setDraft((prev) => ({ ...prev, frequency: value }))}>
                <SelectTrigger id="custom-repeat-frequency" className="bg-muted/50 dark:bg-muted" data-testid="custom-repeat-frequency">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                ...
                {WEEKDAYS.map((day, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-10 h-10 rounded-full text-sm font-semibold transition-all ${active ? 'bg-hub text-white shadow-sm' : 'bg-muted text-foreground/80 dark:text-muted-foreground hover:bg-muted'}`}
                    >
                      {day.label}
                    </button>
                  );
          ...
          >
            ...
            <div className="grid gap-3">
              <button
                ...
                className={`flex items-center gap-3 rounded-lg border px-4 py-3 text-left ${draft.end_mode === 'never' ? 'border-hub-soft bg-hub-soft/30 dark:border-hub-soft' : 'border-border bg-white dark:bg-card'}`}
              >
                <div className={`w-4 h-4 rounded-full border ${draft.end_mode === 'never' ? 'border-4 border-hub' : 'border-border'}`} />
                <span className="text-sm font-medium text-foreground">Never</span>
              </button>

              <div className={`rounded-lg border px-4 py-3 ${draft.end_mode === 'on_date' ? 'border-hub-soft bg-hub-soft/30 dark:border-hub-soft' : 'border-border bg-white dark:bg-card'}`}>
                <button
                  type="button"
                  role="radio"
                ...
                >
                  <div className={`w-4 h-4 rounded-full border ${draft.end_mode === 'on_date' ? 'border-4 border-hub' : 'border-border'}`} />
                ...
                {draft.end_mode === 'on_date' && (
                  <Input
                    ...
                    className="mt-3 bg-white dark:bg-card"
                    data-testid="custom-repeat-end-date-input"
                  />
                )}
              </div>

              <div className={`rounded-lg border px-4 py-3 ${draft.end_mode === 'after_occurrences' ? 'border-hub-soft bg-hub-soft/30 dark:border-hub-soft' : 'border-border bg-white dark:bg-card'}`}>
                <button
                  type="button"
                  role="radio"
                  ...
                  className="flex items-center gap-3 w-full text-left"
                >
                  <div className={`w-4 h-4 rounded-full border ${draft.end_mode === 'after_occurrences' ? 'border-4 border-hub' : 'border-border'}`} />
                ...
                {draft.end_mode === 'after_occurrences' && (
                  <div className="mt-3 flex items-center gap-3">
                    <Input
                      ...
                      className="max-w-[160px] bg-white dark:bg-card"
        ...
        <DialogFooter className="gap-2">
          ...
          <Button type="button" onClick={handleSave} className="bg-hub hover:bg-hub-strong text-white" data-testid="custom-repeat-save-button">
            Done
          </Button>
        </DialogFooter>
      </DialogContent>
